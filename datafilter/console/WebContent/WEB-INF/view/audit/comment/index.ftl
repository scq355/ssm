				...
				<div class="col-sm-12">
					<div class="ibox float-e-margins">
						...
						<div class="select-keywords-comment col-lg-5">
							...
							<input type="checkbox" name="cnm" id="imgchoose" value="" />
							<span>有图</span>
				...
				<div class="col-sm-12">
					...
					<div class="ibox-content">
						...
						<tr>
							...
							<button class="btn btn-info btn-sm" id="dif-back" onclick="backAll()">恢复</button>
...
<script>
	var mySwiper = new Swiper('.swiper-container', {
		...
		//操作swiper之后，是否禁止autoplay。默认为true：停止.
...
<script type="text/javascript">
	...
	$('#freeze-submit').click(function() {
		...
		var urlArr;
		if(getValFree == '1') {
			urlArr = "/comment/batchFreezeId.do"
		} else if(getValFree == '2') {
			urlArr = "/comment/batchFreezeIp.do"
		}
		$.ajax({
			type: "GET",
			dataType: "json",
...
//批量冻ID
	...
	function adoptAll() {
		...
		if(chosse == true) {
			$.ajax({
				type: "GET",
				dataType: "json",
				url: "/comment/batchUpdateStatus.do",
				data: {
					"status": 2,
					"ids": ids,
					"operation": 0
	...
	function searchUsers(userId) {
		window.location = "/comment/look.do?userId=" + userId;
	}
	//查看图片-true
