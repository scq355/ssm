			...
			<div class="row">
				<div class="col-sm-12">
					<div class="ibox float-e-margins">
						...
						<div class="select-keywords-comment col-lg-6">
							<label for="">数据类型：</label>
							<select name="" style="width: 80px;height: 22px;">
...
<script type="text/javascript">
	...
	var listTable = $("#list").dataTable({
		...
		aoColumns: [{
			...
			},
			{
				...
				"fnRender": function(a) {
					...
					var createdAt;
					var creator;
					if(a.aData["auditLogs"][0] != undefined) {
						if(test == '3') {
		...
		bServerSide: true,
		sAjaxSource: "/investment/list.do",
	...
	$('#freeze-submit').click(function() {
		...
		var urlArr;
		if(getValFree == '1') {
			urlArr = "/investment/batchFreezeId.do"
		} else if(getValFree == '2') {
			urlArr = "/investment/batchFreezeIp.do"
		}
		$.ajax({
			type: "GET",
			dataType: "json",
	...
	function freezeID() {
		...
		$(document).on('click', '#freeze-submit', function() {
			...
			$.ajax({
				type: "GET",
				dataType: "json",
				url: "/investment/batchFreezeId.do",
				data: {
					"users": users,
					"userIds": userId,
					"ids": ids,
	...
	function freezeIP() {
		...
		$(document).on('click', '#freeze-submit', function() {
			...
			$.ajax({
				type: "GET",
				dataType: "json",
				url: "/investment/batchFreezeIp.do",
				data: {
					"users": users,
					"userIds": userId,
					"ids": ids,
	...
	$('#refuse-details').click(function() {
		...
		if(chosse == true) {
			$.ajax({
				type: "GET",
				dataType: "json",
				url: "/investment/updateStatus.do",
				data: {
					"status": 3,
					"auditId": ids,
					"operation": 1,
	...
	function adoptAll() {
		...
		if(chosse == true) {
			$.ajax({
				type: "GET",
				dataType: "json",
				url: "/investment/batchUpdateStatus.do",
				data: {
					"status": 2,
					"ids": ids,
					"operation": 0
	...
	function searchUsers(userId) {
		window.location = "/investment/look.do?userId=" + userId;
	}
	//查看图片
