				...
				<div class="col-sm-12">
					...
					<div class="ibox-content">
						...
						<tr>
							...
							<button class="btn btn-info btn-sm" onclick="refuseAll()">拒绝</button>
...
//点击提交冻单个ID/IP
	$('#freeze-submit').click(function() {
		...
		var urlArr;
		if(getValFree == '1') {
			urlArr = "/market/batchFreezeId.do"
		} else if(getValFree == '2') {
			urlArr = "/market/batchFreezeIp.do"
		}
		$.ajax({
			type: "GET",
			dataType: "json",
	...
	function adoptAll(){
		...
		if(chosse == true) {
			$.ajax({
				type: "GET",
				dataType: "json",
				url: "/market/batchUpdateStatus.do",
				data: {
					"status": 2,
					"ids": ids,
					"operation": 0
...
function searchUsers(userId){
    window.location = "/market/look.do?userId="+userId;
}
	//查看图片
