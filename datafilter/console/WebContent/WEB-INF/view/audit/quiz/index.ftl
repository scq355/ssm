...
//点击提交冻单个ID/IP
	$('#freeze-submit').click(function() {
		...
		var urlArr;
		if(getValFree == '1') {
			urlArr = "/quiz/batchFreezeId.do"
		} else if(getValFree == '2') {
			urlArr = "/quiz/batchFreezeIp.do"
		}
		$.ajax({
			type: "GET",
			dataType: "json",
...
function searchUsers(userId){
    window.location = "/quiz/look.do?userId="+userId;
}
	//查看图片
