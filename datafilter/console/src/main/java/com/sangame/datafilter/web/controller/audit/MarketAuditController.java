...
import com.sangame.datafilter.service.audit.FilterMarketAuditService;
...
public class MarketAuditController {
    @Autowired
    FilterMarketAuditService filterMarketAuditService;
    @Autowired
    FilterBlackUserService filterBlackUserService;

    @RequestMapping(value="/index.do", method= RequestMethod.GET)
    public String index(Model model){
        int check = filterMarketAuditService.getDatasCountByStatus(CommonConstant.DataStatus.CHECK.getValue());
        int sysPass = filterMarketAuditService.getDatasCountByStatus(CommonConstant.DataStatus.SYS_PASS.getValue());
        int manualPass = filterMarketAuditService.getDatasCountByStatus(CommonConstant.DataStatus.MANUAL_PASS.getValue());
        int reject = filterMarketAuditService.getDatasCountByStatus(CommonConstant.DataStatus.REJECT.getValue());
        model.addAttribute("checkCount",check);
        model.addAttribute("passCount",sysPass+manualPass);
        model.addAttribute("rejectCount",reject);
        return PageConstant.MARKET_INDEX_PAGE;
    }
    ...
    public Render index(FilterMarketAudit filterMarketAudit){
        Render render = filterMarketAuditService.searchData(filterMarketAudit);
        return render;
    }
    ...
    public Render updateStatus(Integer status,Long auditId,FilterAuditLog filterAuditLog) throws CloneNotSupportedException{
        ...
        filterMarketAuditService.updateStatus(status,auditId,filterAuditLog);
    ...
    public Render batchUpdateStatus(Integer status,String ids, FilterAuditLog filterAuditLog) throws CloneNotSupportedException{
        ...
        filterMarketAuditService.batchUpdateStatus(status,ids,filterAuditLog);
    ...
    public Render batchFreezeId(String users,String userIds,String ids,Boolean dealDataFlag){
        ...
        filterMarketAuditService.dealHistoryData(dealDataFlag,userIds,ids);
