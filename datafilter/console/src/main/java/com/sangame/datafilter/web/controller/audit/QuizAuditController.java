...
import com.sangame.datafilter.service.audit.FilterQuizAuditService;
...
public class QuizAuditController {
    @Autowired
    FilterQuizAuditService filterQuizAuditService;
    @Autowired
    FilterBlackUserService filterBlackUserService;

    @RequestMapping(value="/index.do", method= RequestMethod.GET)
    public String index(Model model){
        int check = filterQuizAuditService.getDatasCountByStatus(CommonConstant.DataStatus.CHECK.getValue());
        int sysPass = filterQuizAuditService.getDatasCountByStatus(CommonConstant.DataStatus.SYS_PASS.getValue());
        int manualPass = filterQuizAuditService.getDatasCountByStatus(CommonConstant.DataStatus.MANUAL_PASS.getValue());
        int reject = filterQuizAuditService.getDatasCountByStatus(CommonConstant.DataStatus.REJECT.getValue());
        model.addAttribute("checkCount",check);
        model.addAttribute("passCount",sysPass+manualPass);
        model.addAttribute("rejectCount",reject);
        return PageConstant.QUIZ_INDEX_PAGE;
    }
    ...
    public Render index(FilterQuizAudit filterQuizAudit){
        Render render = filterQuizAuditService.searchData(filterQuizAudit);
        return render;
    }
    ...
    public Render updateStatus(Integer status,Long auditId,FilterAuditLog filterAuditLog) throws CloneNotSupportedException{
        ...
        filterQuizAuditService.updateStatus(status,auditId,filterAuditLog);
    ...
    public Render batchUpdateStatus(Integer status,String ids, FilterAuditLog filterAuditLog) throws CloneNotSupportedException{
        ...
        filterQuizAuditService.batchUpdateStatus(status,ids,filterAuditLog);
    ...
    public Render batchFreezeId(String users,String userIds,String ids,Boolean dealDataFlag){
        ...
        filterQuizAuditService.dealHistoryData(dealDataFlag,userIds,ids);
